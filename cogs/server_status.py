...
class honCMD():
    ...
    def playerCount_pid(self):
        ...
        check.terminate()
        return i
    def playerCount_pid_raw(self,pid):
        pid = str(pid)
        check = subprocess.Popen([processed_data_dict['player_count_exe_loc'],pid],stdout=subprocess.PIPE, text=True)
        i = int(check.stdout.read())
        check.terminate()
        return i
   ...
   #   Starts server
    ...
    def startSERVER(self,reason):
        ...
        if honCMD.check_proc(processed_data_dict['hon_file_name']) and ('hon_pid' not in self.server_status or self.server_status['hon_pid'] != 'pending'):
            proc_found = False
            print("detected already running hon instance, attempting to hook on..")
            for proc in psutil.process_iter():
                if proc.name() == processed_data_dict['hon_file_name']:
                    #   terminate processes which are reporting a playercount of -1. This means it's failed to launch
                    #   Healthy playercount is either -3 (off) or >= 0 (alive)
            ...
            for proc in psutil.process_iter():
                if proc.name() == processed_data_dict['hon_file_name']:
                    if self.playerCount_pid_raw(proc.pid) >= 0:
                        print("found running hon instance")
